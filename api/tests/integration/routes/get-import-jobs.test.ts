...
describe("GET /imports route integration tests", async () => {
  ...
  await test("GET /imports route returns an array with 2 groups of jobs", async (t: TestContext) => {
    t.plan(1);

    await seed(database, { jobs }).refine((functions) => ({
      jobs: {
        columns: {
          name : functions.valuesFromArray({ values: ["import"] }),
  ...
  await test("GET /imports route returns all pending jobs in the pending group", async (t: TestContext) => {
    t.plan(1);
  ...
  await test("GET /imports route returns all finished jobs in the finished group", async (t: TestContext) => {
    t.plan(1);
  ...
  await test("GET /imports route does not return any export job", async (t: TestContext) => {
    t.plan(2);
    ...
    const importJobsCount = await database.$count(jobs, eq(jobs.name, "import"));
    const response        = await app.inject({
      method: "GET",
      url   : "/imports",
    }) as { json: () => { jobs: Job[]; state: string }[] };
    ...
    t.assert.strictEqual(returnedExportJobs.length, 0);
    t.assert.strictEqual(returnedImportJobs.length, importJobsCount);
  });
});
